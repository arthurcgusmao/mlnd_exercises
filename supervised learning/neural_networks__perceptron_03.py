...
Network = [
    ...
    [Perceptron(np.array([0,1]), 0.5), Perceptron(np.array([1,0]), 0.5), Perceptron(np.array([1,1]), 1.5)],
    # output node, declare output layer perceptron here
    [Perceptron(np.array([1,1,-3]))]
]
...
def EvalNetwork(inputValues, Network):
    ...
    for layer in Network:
        ...
        for perceptron in layer:
            OutputValues.append(perceptron.activate(inputValues))
        inputValues = OutputValues

    if len(OutputValues) == 1:
        OutputValue = OutputValues[0]
    else:
        raise Exception('There is not a single perceptron in the output layer')
