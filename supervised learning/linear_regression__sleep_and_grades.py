...
def compute_regression(sleep,scores):
    ...
    normalized_sleep = np.vectorize(lambda x: x - avg_sleep)(sleep)
    normalized_scores = np.vectorize(lambda x: x - avg_scores)(scores)
    ...
    slope = np.dot(normalized_sleep, normalized_scores) / np.sum(normalized_sleep**2)
